...
- **No cross-context imports of internals** in the skeleton (aggregates, repositories, VOs, `*Handler`/`*UseCase`/`*Subscriber`). If the context needs data from `User`, react to its domain events or dispatch its published `*Command`/`*Query` through the bus (the `PublicMessage` deptrac layer) — never import its handler or aggregate.
...
- **Foreign keys are real `#[ORM\ManyToOne]` associations** (`#[ORM\JoinColumn(name: '<x>_id', onDelete: 'CASCADE')]`) — Doctrine owns the FK + index, so `migrate-diff` stays stable. `*Model` classes are **not `final`** (Doctrine subclasses them for proxies; the `final_class` cs-fixer rule exempts `#[ORM\Entity]`). A **cross-context** FK (association to another context's `*Model`) is allowed at the Persistence boundary only — add a `deptrac.yaml` `skip_violations` entry for the `*Model` and any repository calling `getReference(OtherModel::class, …)`, and filter reads with `IDENTITY(x.assoc)` so read repositories stay import-free (see `.ai/lessons.md` L-009).
