...
| 2026-07-23 | Implemented. Resolved the `final` RefreshToken lazy-ghost blocker (drop `final` + build-time `cache:warmup`). All gates green: `lint-api`, `test-api` (58), `gen-api` (no drift), `lint-web`, `test-web`, `test-e2e` (10, worker mode), runtime smoke (`/healthz` 200, `/` 404, non-root, 8 extensions). Code review clean on both axes (standards + spec). |
